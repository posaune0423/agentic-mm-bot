...
if (!sdkPath) {
  console.warn(
    "Could not find extended-typescript-sdk in standard locations. Skipping WASM fix.",
  );
...
const potentialWasmDirs = [
  path.join(sdkPath, "dist/wasm"),
  path.join(sdkPath, "wasm"),
  path.join(sdkPath, "pkg"),
];

let wasmSrcDir = potentialWasmDirs.find(
...
if (!wasmSrcDir) {
  ...
  // Check again
  wasmSrcDir = potentialWasmDirs.find(
    (d) => fs.existsSync(d) && fs.readdirSync(d).length > 0,
  );
}
...
if (wasmSrcDir) {
  ...
  console.error(
    "Could not locate or build WASM files. Apps requiring them may fail.",
  );
