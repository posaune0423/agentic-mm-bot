...
describe("ProposalRepository.getCurrentParams", () => {
  it("should return default params when no current params", async () => {
    const db = createMockDb();
    ...
    // When DB is empty, repository seeds a default strategy_params row via transaction.
    const seededParams = {
      id: "seeded-params-1",
      exchange: "extended",
      symbol: "BTC-USD",
    ...
    const txInsertMock = mock(() => ({
      values: mock(() => ({
        returning: mock(() => Promise.resolve([seededParams])),
      })),
    }));

    // Override transaction to use the mocks that return seeded params
    db.transaction = mock(async (fn: (tx: any) => Promise<any>) =>
      fn({
        update: txUpdateMock,
        insert: txInsertMock,
      }),
    ...
    if (result.isOk()) {
      ...
      expect(result.value.symbol).toBe("BTC-USD");
      expect(result.value.isCurrent).toBe(true);
      expect(result.value.createdBy).toBe("system");
    }
  });
